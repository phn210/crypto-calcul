...
void test_sha2()
{
    ...
    check &= test(vectors[0][0], digest, SHA224_DIGEST_SIZE);
    sha224((const unsigned char *)message2a, strlen(message2a), digest);
    check &= test(vectors[0][1], digest, SHA224_DIGEST_SIZE);
    sha224(message3, message3_len, digest);
    check &= test(vectors[0][2], digest, SHA224_DIGEST_SIZE);
    if (check)
    {
        printf("\tPASSED\n");
    }
    ...
    check &= test(vectors[1][0], digest, SHA256_DIGEST_SIZE);
    sha256((const unsigned char *)message2a, strlen(message2a), digest);
    check &= test(vectors[1][1], digest, SHA256_DIGEST_SIZE);
    sha256(message3, message3_len, digest);
    check &= test(vectors[1][2], digest, SHA256_DIGEST_SIZE);
    if (check)
    {
        printf("\tPASSED\n");
    }
    ...
    check &= test(vectors[2][0], digest, SHA384_DIGEST_SIZE);
    sha384((const unsigned char *)message2b, strlen(message2b), digest);
    check &= test(vectors[2][1], digest, SHA384_DIGEST_SIZE);
    sha384(message3, message3_len, digest);
    check &= test(vectors[2][2], digest, SHA384_DIGEST_SIZE);
    if (check)
    {
        printf("\tPASSED\n");
    }
    ...
    check &= test(vectors[3][0], digest, SHA512_DIGEST_SIZE);
    sha512((const unsigned char *)message2b, strlen(message2b), digest);
    check &= test(vectors[3][1], digest, SHA512_DIGEST_SIZE);
    sha512(message3, message3_len, digest);
    check &= test(vectors[3][2], digest, SHA512_DIGEST_SIZE);
    if (check)
    {
        printf("\tPASSED\n");
    }
